...
import numpy as np
from drawables import DirectionalLight, PointLight, SpotLight, Texture
...
class SceneGraph:
    ...
    def get_transform(self, node):
        ...
        rotation_matrix = tr.rotationY(node["rotation"][1]) @ tr.rotationX(node["rotation"][0]) @ tr.rotationZ(
            node["rotation"][2])
                ...
                for pipeline in current_pipelines:
                    ...
                    if isinstance(current_node["light"], DirectionalLight):
                        ...
                        if "u_numPointLights" in pipeline.uniforms:
                            ...
                            position = (self.transformations[src] @ np.array(
                                [current_node["position"][0], current_node["position"][1], current_node["position"][2],
                            ...
                            pipeline[f"u_pointLights[{str(pointLightIndex)}].quadratic"] = current_node[
                                "light"].quadratic
                        ...
                        if "u_numSpotLights" in pipeline.uniforms:
                            ...
                            pipeline[f"u_spotLights[{str(spotLightIndex)}].direction"] = (self.transformations[
                                                                                              src] @ self.get_forward(
                                dst))[:3]
                            ...
                            pipeline[f"u_spotLights[{str(spotLightIndex)}].outerCutOff"] = current_node[
                                "light"].outerCutOff
